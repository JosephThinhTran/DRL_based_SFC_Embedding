...
Changelog:
    + Based on the DQ_SFC_Main_01.08.py
...
from q_models_gpu_CONUS import DQAgent # Using GPU for training
...
req_json_file = os.path.join(DATA_FOLDER, "non_uniform_CONUS36_reordered_traffic_500000_slots_1_con.tra")  # service request traffic
# Create EDF environment
edf = EdfEnv(data_path=DATA_FOLDER, 
             net_file='non_uniformCONUS36_500000_slots_1_con.net', 
...
# Training parameters
N_EPOCHS = 150_000 if not IS_CONTINUE_TRAINING else 100_000
BUFFER_SIZE = 500_000
BATCH_SIZE = 200
SYNC_FREQ = 100
...
print("Copy trained Q-network to the TRAIN_DIR folder")
src_file = os.path.join(MODEL_DIR, 'QNet.pt')
shutil.copy2(src_file, TRAIN_DIR)
src_file = os.path.join(MODEL_DIR, 'TargetQNet.pt')
shutil.copy2(src_file, TRAIN_DIR)
...
moving_avg_loss = data_utils_CONUS_02.mov_window_avg(losses, 1000)
...
reject_req_file = os.path.join(TRAIN_DIR, reject_req_file)
