...
class Simulation:
    ...
    def __init__(self, network, sfc_list):
        ...
        self.parameters = {'unit_test': False}

    ########################################################################
    def append_values_to_curves(self, t, curves, throughput, offered_load_every_slot_list,
        ...
        cpu_usage, ram_usage, disk_usage, bw_usage = self.get_resource_usage()
        ...
        # if t % self.parameters['window_size'] == 0:
    ...
    def add_event(self, event):
        try:
            if self.parameters['unit_test'] is True:
                for e in self.event_queue.queue:
                    if e[0] == (event["time_point"], event['sfc_request']['id']):
                        assert False, "duplicated event time and id"
    ...
    def run(self, save_network_stats, save_processed_events, print_simulation_to_file, save_curves):
        ...
        while not self.event_queue.empty():
            try:
                event = self.process_next_event(print_simulation_to_file)
            except BaseException as err:
                print(f"Unexpected {err=}, {type(err)=}")
                print('Simulation::run()', datetime.now().time())
                assert False

            if save_network_stats and event[1]['time_point'] <= self.parameters['capacity_decision_point']:
                self.collect_running_network_stats()

            if print_simulation_to_file and len(self.processed_events) % 5000 == 0:
                self.print_simulation_to_file()

            if save_curves and len(self.processed_events) % 500 == 0:
    ...
    def process_new_request_event(self, an_event):
        ...
        # if an_event["time_point"] == 5169:
        #    print("simulation::process_new_request_event")
        ...
        try:
            ...
            print(f"Unexpected {err=}, {type(err)=}")
            print("process_new_request_event()", datetime.now().time())
            assert False  # , "simulation.py::process_new_request_event"
    ...
    ########################################################################
    # process 'accept' event, i.e., reserving resources for the accepted request
        ...
        try:
            ...
            for i in range(len(path) - 1):  # reserve bandwidth and compute resources along the path
                ...
                if uu[1] == vv[1]:  # uu and vv are at the same layer
                    ...
                    self.network.nodes[vnf_location]["free_CPU"] -= sfc_request[vnf['id']]['CPU_req']
                    # self.network.nodes[vnf_location]["free_RAM"] -= bw * vnf["RAM_rate"]
                    self.network.nodes[vnf_location]["free_RAM"] -= sfc_request[vnf['id']]['RAM_req']
                    # self.network.nodes[vnf_location]["free_MEM"] -= bw * vnf["MEM_rate"]
                    self.network.nodes[vnf_location]["free_STO"] -= sfc_request[vnf['id']]["STO_req"]
                    assert self.network.nodes[vnf_location]["free_CPU"] >= 0, "free_CPU is not > 0"
                    assert self.network.nodes[vnf_location]["free_RAM"] >= 0, "free_RAM is not > 0"
                    if self.network.nodes[vnf_location]["free_STO"] < 0:
                        assert False
        except BaseException as err:
            print(f"Unexpected {err=}, {type(err)=}")
            print("process_next_event()", datetime.now().time())
            assert False, "simulation.py::process_accept_event"
    ...
    def process_next_event(self, print_simulation_to_file):
        try:
            ...
            try:
                ...
                print("Cannot get next_event")
                print(f"Unexpected {err=}, {type(err)=}")
                print("process_next_event()", datetime.now().time())
                for e1 in self.event_queue.queue:
                    for e2 in self.event_queue.queue:
                        assert e1[0] != e2[0], "Keys must be different"
            ...
            try:
                ...
                print(f"Unexpected {err=}, {type(err)=}")
                print('Simulation::process_next_event()', datetime.now().time())
                print("Unexpected error:", sys.exc_info()[0])
                assert False

            return next_event  # return event to control the stat recording functions
        except BaseException as err:
            print(f"Unexpected {err=}, {type(err)=}")
            print("process_next_event()", datetime.now().time())
            assert False
    ...
    ########################################################################
    def make_new_label_GLSA(self, layered_graph, sfc_request, u, v, prefix_path_cost_and_weight, detailed_path,
        ...
        lower_bound_delay = prefix_path_cost_and_weight[1] + layered_graph[u][v]['delay'] + self.get_remaining_delay(
            v[0], v[1], sfc_request)
        ...
        if u[1] == v[1]:  # u and v are on the same layer
            ...
            try:
                ...
                ram_usage = node_usage[original_node]['RAM'] + sfc_request[vnf]['RAM_req']
                ...
                storage_usage = node_usage[original_node]['MEM'] + sfc_request[vnf]['STO_req']
                if storage_usage > self.network.nodes[original_node]['free_STO']:
                    return none_value_set

                new_node_usage[original_node] = {'CPU': cpu_usage, 'RAM': ram_usage, 'MEM': storage_usage}
            except BaseException as err:
                print(f"Unexpected {err=}, {type(err)=}")
                print('Simulation::process_terminating_event()', datetime.now().time())
                assert False
    ...
    def collect_running_network_stats(self):
        ...
        for node in self.network.nodes:
            if self.network.nodes[node]['VNF_list']:
                ...
                used_sto = self.network.nodes[node]['STO_cap'] - self.network.nodes[node]['free_STO']
                self.max_node_used_cap[node]['STO'] = max(self.max_node_used_cap[node]['STO'], used_sto)
    ...
    def initialize_network_stats(self):
        ...
        for node in self.network.nodes:
            ...
            self.max_node_used_cap[node]['STO'] = 0
        ...
        try:
            ...
            for i in range(len(layered_graph_path) - 1):
                ...
                if u[1] == v[1]:
                    ...
                    vnf_location = u[0]
                    self.network.nodes[vnf_location]["free_CPU"] += sfc_request[processing_vnf['id']]["CPU_req"]
                    # self.network.nodes[vnf_location]["free_RAM"] += bw * processing_vnf["RAM_rate"]
                    self.network.nodes[vnf_location]["free_RAM"] += sfc_request[processing_vnf['id']]["RAM_req"]
                    # self.network.nodes[vnf_location]["free_MEM"] += bw * processing_vnf["MEM_rate"]
                    self.network.nodes[vnf_location]["free_STO"] += sfc_request[processing_vnf['id']]["STO_req"]
                    ...
                    assert self.network.nodes[vnf_location]["free_STO"] <= self.network.nodes[vnf_location]["STO_cap"]
        except BaseException as err:
            print(f"Unexpected {err=}, {type(err)=}")
            print('Simulation::process_terminating_event()', datetime.now().time())
            assert False
    ...
    def get_total_resource_capacities(self):
        ...
        for u in self.network.nodes:
            ...
            total_disk_cap += self.network.nodes[u]['STO_cap']
    ...
    def get_resource_usage(self):
        ...
        for u in self.network.nodes:
            ...
            total_disk_usage += self.network.nodes[u]['STO_cap'] - self.network.nodes[u]['free_STO']
    ...
    def compute_shortest_distances_between_all_pairs(self):
        ...
        for s in self.network.nodes:
            ...
            for d in self.network.nodes:
                if s != d:
                    try:
                        self.shortest_distances[s][d] = networkx.dijkstra_path_length(self.network, s, d,
                                                                                      weight='delay')  # the shortest length
                    except networkx.NetworkXException:
    ...
    def print_curves_to_file(self):
        ...
        total_accepted_requests, total_requests = 0, 0

        print('Simulation for curves is running ... ',
              datetime.now().time())  # this simulation run with 'accept' and 'terminate' events only
        ...
        for event_time_and_request_id, event in sorted_event_list:
            ...
            if event['type'] == 'accept':
                ...
                last_acceptance_rate = total_accepted_requests / total_requests * 100.0
                ...
                if t == 6000 or t % self.parameters['window_size'] == 0:  # reset counting at a steady point
                    total_accepted_requests = total_requests = 0
            ...
            if event['type'] == 'accept':  # or event['type'] == 'terminate':
                self.append_values_to_curves(t, curves, through_put, offered_load_every_slot_list,
        ...
        # plt.xlabel("Time slot")
        # # plt.annotate(str(last_acceptance_rate), xy=(last_accept_point, last_acceptance_rate))
