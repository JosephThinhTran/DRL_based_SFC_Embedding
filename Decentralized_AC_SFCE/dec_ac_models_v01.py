# -*- coding: utf-8 -*-
"""
Created on Mon Feb 21 23:50:30 2022

@author: Onlyrich-Ryzen

Implement Decentralized Actor-Critic Models

- Change log:
    + Modified from actor_critic_models_SFC_v05.py
...
from edf_env_MARL_v1_01 import EdfEnv # from edf_env_A3C_v2_02 import EdfEnv
from data_utils_MARL_01 import mov_window_avg, delete_data, calc_throughput, numpy_encoder, find_runs
...
class ACNet(torch.nn.Module):
    def __init__(self, name, model_dir, 
                fc_hid_layers, input_dims, actor_dims, net_arch,
                train_dev='cpu'):
        '''
        ...
        if torch.cuda.is_available() and self.train_dev == 'gpu':
            print("Using GPU for training ACNet")
            self.device = 'cuda:0'
        else:
            print("Using CPU for Training ACNet")
            self.device = 'cpu'
        self.to(self.device)
...
class A3CWorker(mp.Process):
        ...
        for epoch in range(self.max_episodes):
            ...
            if self.is_train and ((epoch+1) % self.train_freq == 0):
                ...
                with self.g_counter.get_lock():
                    ...
                    if torch.cuda.is_available() and self.train_dev == 'gpu':
                        #print("GPU training")
                        actor_loss, critic_loss = self.update_params_batch_gpu()# Train on GPU
                    else:
                        # print("CPU training")
                        actor_loss, critic_loss = self.update_params_batch() #Train on CPU
    ...
    def run_episode(self, epoch):
        ...
        #### While loop (Rollout-based SFC embedding and routing)
    ...
    def update_params_batch_gpu(self):
        """
        Compute and minimizing the loss for updating the model
