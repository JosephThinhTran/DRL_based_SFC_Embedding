...
from tf_DQN_agent import TFDQAgent  # DQN agent using TensorFlow
...
req_json_file = os.path.join(DATA_FOLDER, "reordered_traffic_100000_slots_1_con.tra")  # service request traffic
...
q_agent = TFDQAgent(model_dir=MODEL_DIR,
                    fc_hid_layers=FC_HID_LAYERS,
                    input_dims=INPUT_DIMS,
                    output_dims=OUTPUT_DIMS,
                    gamma=GAMMA,
...
for epoch in range(N_EPOCHS):
    ...
    while(stop_flag < 1): 
        ...
        print('action_raw:', action_raw, '; action:', action)
