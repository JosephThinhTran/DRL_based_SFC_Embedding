...
- Changelog from version 01.06:
    ...
    + Use edf_env_v2_01.py due to the new dataset's format
"""
...
DATA_FOLDER = r"SOF_Data_Sets-v09\complete_data_sets"
traffic_file_name = "reordered_traffic_15000_slots_1_con.tra"
req_json_file = os.path.join(DATA_FOLDER, traffic_file_name)  # service request traffic
edf = EdfEnv(data_path=DATA_FOLDER, net_file='ibm_15000_slots_1_con.net', sfc_file='sfc_file.sfc')# EDF environment
...
# Training parameters
N_EPOCHS = min(15_000, n_req)#  number of requests for testing
BUFFER_SIZE = 10_000
BATCH_SIZE = 200
SYNC_FREQ = 100
...
with open(regis_file, 'w') as rf:
    json.dump(registered_req, rf, indent=2, default=data_utils_02.numpy_encoder)
...
for req_item in registered_req['registered_request']:
    ...
    n_diff_nodes, _, _ = data_utils_02.find_runs(req_item['sfc_map'])
